...
<head>
	...
	*{margin: 0; padding: 0;}
	...
	table td{text-align:center; font:normal 12px '나눔바른고딕';}
	...
	#write>a{margin-left:820px;}
           ...
           .tableSubject>a{font-weight:bold; color:#066978;}
...
<body>
	<div id="wrap"> <!--**** #wrap start ****-->
		...
		<div id="title_img">
			<h2>
				<img src="../images_sabujak/banner2.png" width="1260px"/>
			</h2>
		</div>
		<div id="produck_sort">
			<ul class="sort">
				...
				<c:if test="${!empty loginUser}">
					<li><a href="sabujak.do?m=write" class="last-line"><span>글쓰기</span></a></li>
				</c:if>
			</ul>
		</div>
        ...
        <table width="750px">
            ...
            <thead>
                <tr>
                    ...
                    <th style="background:white;border-bottom:2px solid #333;background:url('/src/images/contents/sub_th_bg.gif') no-repeat left center #F8F8F8;padding:8px 0px !important;">조회수</th>
                </tr>
				...
				<c:forEach items="${listResult.list}" var="board">
					<tr>
                  	<td>${board.c_no}</td>
                  	<td class="tableSubject"><a href="sabujak.do?m=content&c_no=${board.c_no}">${board.c_sub}</a></td>
                  	<td>${board.email_fk_pk}</td>
                  	<td>${board.c_date}</td>
                  	<td>${board.c_view}</td>
                  	</tr>
                </c:forEach>
...
<hr width='750px' size='2' color='gray' noshade>
<font color='gray' size='3' face='나눔바른고딕'>
    ...
    (Total Post : ${listResult.totalCount} )
